...
TEST (JsonRectangleReader, invalid_data) {
    EXPECT_THROW (vector<Rectangle> result = JsonRectangleReader(invalid_data_file_name).read(), JsonRectangleReader::InvalidDataError);
}

TEST (JsonRectangleReader, parse_error) {
    EXPECT_THROW (vector<Rectangle> result = JsonRectangleReader(parse_error_file_name).read(), JsonRectangleReader::ParsingError);
}
